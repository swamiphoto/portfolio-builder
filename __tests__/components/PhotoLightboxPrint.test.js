...
// production). Wrap the lightbox in it so clicking the affordance can open it.
...
it('opens the configurator drawer when the affordance is clicked', () => {
  renderInStore(<PhotoLightbox images={[sellable]} index={0} onClose={() => {}} onNavigate={() => {}} printStore={{ enabled: true, markup: 3 }} />, { enabled: true, markup: 3 })
  // Drawer is closed (aria-hidden) → its Buy CTA is not in the a11y tree yet.
