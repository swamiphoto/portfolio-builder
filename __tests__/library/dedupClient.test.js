...
describe('backfillHashes', () => {
  it('fingerprints every asset from one storage listing (etag by publicUrl)', async () => {
    global.fetch.mockReturnValueOnce(json({ hashes: { u1: 'etag1', u2: 'etag2' } }))
    const assets = {
      c1: { assetId: 'c1', publicUrl: 'u1' },
      c2: { assetId: 'c2', publicUrl: 'u2' },
      c3: { assetId: 'c3', publicUrl: 'u-missing' }, // not in storage listing
    }
    const progress = []
    const out = await backfillHashes(assets, { onProgress: (p) => progress.push(p) })
    expect(out.hashes).toEqual({ c1: 'etag1', c2: 'etag2' })
    expect(out.failed).toEqual([{ assetId: 'c3', reason: 'no storage fingerprint' }])
    expect(progress[progress.length - 1]).toEqual({ done: 3, total: 3 })
    // ONE listing call, regardless of library size — no per-image downloads
    expect(global.fetch).toHaveBeenCalledTimes(1)
    expect(global.fetch).toHaveBeenCalledWith('/api/admin/dedup/storage-hashes', expect.objectContaining({ signal: undefined }))
  })
})
