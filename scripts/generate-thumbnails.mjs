/**
 * Generate 600px-wide JPEG thumbnails for all photos in R2.
 * Thumbnails stored at users/{userId}/thumbnails/{original-path}
 * (mirrors the photos/ path, just under thumbnails/)
 *
 * Run: node scripts/generate-thumbnails.mjs
 * Re-running is safe — skips thumbnails that already exist.
 */
...
async function main() {
  ...
  const tasks = assets.map(asset => async () => {
    ...
    const thumbKey = `users/${userId}/thumbnails/${suffix.replace(/\.[^.]+$/, '.jpg')}`
  ...
  console.log(`Generating thumbnails (${THUMB_WIDTH}px, quality ${THUMB_QUALITY}, concurrency 8)...`)
