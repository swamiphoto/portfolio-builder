              ...
              {sequence.map((item, i) => {
                if (item.type === 'text') {
                  return (
                    <button
                      key={`text-${i}`}
                      onClick={() => item.excludeKey && toggleExcluded(item.excludeKey)}
                      onMouseEnter={(e) => {
                      ...
                      className={`relative group w-11 h-11 bg-stone-100 border rounded flex items-center justify-center flex-shrink-0 transition-all ${
