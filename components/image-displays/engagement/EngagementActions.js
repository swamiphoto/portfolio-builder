...
function HeartIcon({ filled, size = 17 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={filled ? '#c14a4a' : 'none'} stroke={filled ? '#c14a4a' : 'currentColor'} strokeWidth="1.8">
      <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
    </svg>
  )
}

function CommentIcon({ size = 17 }) {
  return (
...
export default function EngagementActions({ imageUrl }) {
  ...
  // Manhattan squares the panel and shrinks it to match the Buy-a-print button.
  const pill = manhattan ? { ...PILL, borderRadius: 0 } : PILL
  const btn = manhattan ? { ...BTN, padding: '5px 9px', fontSize: 11 } : BTN
  const iconSize = manhattan ? 14 : 17
        ...
        <React.Fragment key={section}>
          ...
          {section === 'favorites' && (
            <button
              ...
              style={btn}
            >
              <HeartIcon filled={mine} size={iconSize} />
