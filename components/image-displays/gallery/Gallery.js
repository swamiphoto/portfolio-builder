...
import BlantyreOffsetGallery from "../themes/blantyre/BlantyreOffsetGallery";
...
// Blantyre empty-state palette — deeper sage tiles on the sage ground.
const BL_TILE = '#cfd2c3'
const BL_ICON = '#a3a996'
...
function mobilePlaceholderSkin(themeId) {
  ...
  if (themeId === 'blantyre') return { square: true, bg: BL_TILE, iconColor: BL_ICON }
...
function PlaceholderGrid({ variant = 'masonry', size = 'large', themeId = 'kyoto', mobile = false }) {
  ...
  if (variant === 'offset') {
    // Mirror BlantyreOffsetGallery: a wide tile and a narrow one dropped down.
    return (
      <div className="w-full max-w-6xl mx-auto p-4 md:p-8" data-photos-placeholder="offset">
        <div className="flex justify-between items-start">
          <div style={{ width: '58%' }}><PlaceholderTile aspectClass="aspect-[3/4]" square bg={BL_TILE} iconColor={BL_ICON} /></div>
          <div style={{ width: '30%', marginTop: '16%' }}><PlaceholderTile aspectClass="aspect-[3/4]" square bg={BL_TILE} iconColor={BL_ICON} /></div>
        </div>
      </div>
    )
  }
  ...
  if (variant === 'centered') {
    return (
      <div className="mx-auto w-full px-4 md:px-8" style={{ maxWidth: `${PHOTO_CENTERED_PCT[sz]}%` }} data-photo-placeholder="centered">
        <PlaceholderTile aspectClass="aspect-[3/2]" {...(themeId === 'blantyre' ? { square: true, bg: BL_TILE, iconColor: BL_ICON } : {})} />
      </div>
    )
  }
...
const Gallery = ({ name, description, blocks, enableSlideshow, enableClientView, pages, childPages, activeChildId, currentPageId, username, basePath, onBackClick, onSlideshowClick, onClientLoginClick, onChildPageClick, onPageClick, showPlaceholders, onBlockHover, onBlockClick, siteConfig, printStore, themeId = 'kyoto', hasCover = false, coverHeight = 'partial', coverButtonStyle = 'solid', cover = null, opener = 'title' }) => {
  ...
  const Wiggle = () => (themeId === 'manhattan' || themeId === 'blantyre' ? null : <WiggleLine />)
        ...
        {(blocks || []).map((block, index) => {
          ...
          switch (block.type) {
            case "photos": {
              ...
              // Blantyre's signature staggered scatter (desktop only — phones fall
              ...
              if (variantId === 'offset' && !isSmallScreen) {
                return (
                  <div key={`block-${index}`} className="photos-block" data-block-index={index} {...hoverProps}>
                    <BlantyreOffsetGallery images={imageRefs} onImageClick={makeClickHandler(index)} captionStyle={capStyle(block)} size={size} />
                  </div>
                );
              }
                ...
                return (
                  <div key={`block-${index}`} className="photos-grid-block" data-block-index={index} {...hoverProps}>
                    {themeId === 'manhattan'
                      ...
                      : <GridGallery images={imageRefs} onImageClick={makeClickHandler(index)} basis={GRID_BASIS[size]} edgeToEdge={themeId === 'provence'} rounded={themeId !== 'provence' && themeId !== 'blantyre'} />}
                  </div>
                );
              }
